...
void lv_setting_page(void)
{
    ...
    lv_menu_set_mode_root_back_button(menu, LV_MENU_ROOT_BACK_BTN_ENABLED);
    lv_obj_add_event_cb(menu, navigate_back, LV_EVENT_CLICKED, menu);
    ...
    /*Create sub pages*/
    lv_obj_t * wifi_sub_page = lv_menu_page_create(menu, NULL);
    lv_obj_set_style_pad_hor(wifi_sub_page, lv_obj_get_style_pad_left(lv_menu_get_main_header(menu), 0), 0);
    lv_menu_separator_create(wifi_sub_page);
    section = lv_menu_section_create(wifi_sub_page);
    ...
    lv_menu_set_load_page_event(menu, cont, wifi_sub_page);
